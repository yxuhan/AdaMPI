...
def restore_model(model_path, mpi_predict_network, optimizer, logger, load_part_list=None):
    ...
    key_model_list = [
        ("mpi_predict_network", mpi_predict_network), ("optimizer", optimizer)
    ]

    for key, model in key_model_list:
        ...
        if key == "optimizer" and load_part_list is None:
            model.load_state_dict(_state_dict)
        elif key == "mpi_predict_network":
            if load_part_list is None:
                model.load_state_dict(_state_dict)
            else:
                cur_state_dict = model.state_dict()
                for k in _state_dict.keys():
                    part_name = k.split('.')[0]
                    if part_name not in load_part_list:
                        continue
                    cur_state_dict[k] = _state_dict[k]
                model.load_state_dict(cur_state_dict)

    return state_dict["global_step"], state_dict["current_epoch"]
